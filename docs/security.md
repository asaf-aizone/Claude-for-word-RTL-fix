...
The tool can be started in two ways, both of which end up with the same set of running processes (one or more of Word/Excel/PowerPoint/Outlook, plus a single hidden Node injector serving all four; Outlook is registered but blocked by default and only attaches after explicit Connect Outlook + a per-launch opt-in flag):

1. **Tray icon (recommended).** After `install.bat` runs, a per-user Startup-folder shortcut (`Claude for Word RTL Tray.lnk`, filename retained for v0.1.x upgrade compatibility) launches `scripts/start-tray.vbs` at login, which launches the tray icon. The user opens Word, Excel, or PowerPoint normally and then right-clicks the tray icon and picks **Connect Word**, **Connect Excel**, or **Connect PowerPoint**; the tray enumerates the app's open documents/workbooks/presentations via COM, gracefully closes the app, and relaunches it through the matching wrapper (`word-wrapper.bat`, `excel-wrapper.bat`, or `powerpoint-wrapper.bat`) with the WebView2 debug flag set and the same files reopened. No registry writes, no file-association changes. Fully reversed by `uninstall.bat`. **Connect Outlook** is also available but follows a stricter flow: it shows a per-launch warning dialog (default-Cancel) naming the exposure, and only then writes the per-launch opt-in flag at `%TEMP%\claude-office-rtl.outlook-optin` that the injector requires before attaching to Outlook. See the Outlook-specific section below.
...
6. `scripts/tray-icon.ps1` runs in parallel as a zero-dependency PowerShell tray indicator (green / red / gray) driven by the aggregate status file at `%TEMP%\claude-word-rtl.status` (icon color) and the per-app status file at `%TEMP%\claude-office-rtl.apps.json` (per-app status labels at the top of the menu, one per each of the four supported apps).
...
- **`%TEMP%\claude-word-rtl.status`**, **`.pid`**, **`.tray.pid`**, **`.lock`**, plus **`%TEMP%\claude-office-rtl.apps.json`** (v0.2.0+ per-app state file written by the injector and read by the tray for the four per-app status labels; example shape `{"Word": "...", "Excel": "...", "PowerPoint": "...", "Outlook": "..."}`) - small transient files used for IPC between the injector, the tray, and the four wrappers. All written at runtime only. Cleared by `cleanup.bat` and `uninstall.bat`. The `claude-word-rtl.*` filenames are kept for v0.1.x upgrade compatibility even though the tool now covers all four Office apps; the prefix is a name, not a constraint.
