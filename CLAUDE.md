...
1. User right-clicks the tray icon near the clock. `scripts/tray-icon.ps1` owns the icon and menu. The menu shows four disabled status labels at the top (Word/Excel/PowerPoint/Outlook, each as "connected", "not running", "running without RTL", or "error"), four Connect items below them (Connect Word / Connect Excel / Connect PowerPoint / Connect Outlook - the last is opt-in and shows a warning dialog before launching; the auto-launch path does NOT spawn the injector for Outlook, only for Word/Excel/PowerPoint), a "Disconnect Outlook only" item, and a single Disconnect-all that closes every Office app and the injector.
