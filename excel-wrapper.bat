...
REM Mutex-settle wait. Excel uses a per-user named mutex for single-instance
REM coordination. The mutex can outlive the visible EXCEL.EXE process by
REM 1-3 seconds while WebView2 and COM unwind. If the tray's "Connect Excel"
REM flow relaunches Excel before the mutex is released, the new EXCEL sees
REM an existing instance, hands off to the dying one, and silently exits -
REM looking like "Excel never came back" to the user. Poll tasklist up to ~5s
REM (5 x 1s) for EXCEL.EXE to disappear, then proceed. Fail-open: if
