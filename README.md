...
<p>
<strong>אינו תוסף רשמי של Anthropic או של Microsoft.</strong> כלי open-source עצמאי. שם המאגר ב-GitHub עודכן ב-v0.2.1 ל-<code>Claude-for-Office-RTL-fix</code> (היה <code>Claude-for-word-RTL-fix</code>). GitHub שומר על redirect קבוע מהשם הישן, ולכן <span dir="ltr">clones, bookmarks, and clone URLs</span> מגרסאות v0.1.x ממשיכים לעבוד.
</p>
...
משתמשים בהן, ולכן מערכות EDR ארגוניות <span dir="ltr">(Microsoft Defender for Endpoint, CrowdStrike Falcon, SentinelOne, Sophos)</span> עלולות לזהות את ההתקנה כפעילות חשודה ולנתק את המכונה
...
<p>
  ...
  <br>
  <em>מימין - הפאנל לפני. משמאל - אחרי תיקון <span dir="ltr">RTL</span>. אותה שאלה, אותה תשובה.</em>
</p>
...
<span dir="ltr">(<code>word-wrapper.bat</code>, <code>excel-wrapper.bat</code>, <code>powerpoint-wrapper.bat</code>)</span>, שמגדיר את משתנה ה-WebView2 רק
...
<ul>
  ...
  <li><strong>Redaction של ה-URL בלוג.</strong> ה-URL של תוסף Office מכיל פרמטר <code>et=</code> שהוא base64 של מטא-דאטה של ה-tenant <span dir="ltr">(account id, tenant id, expiry)</span>. עבור Outlook בלבד, ה-injector חותך מהלוג את כל הפרמטרים מלבד <code>_host_Info=</code>. ה-URL של Word/Excel/PowerPoint נשאר ללא שינוי לתאימות אבחון.</li>
</ul>
<p>
...
<p><strong>תמיכה ב-<span dir="ltr">Office Online</span>, ב-<span dir="ltr">Mac</span>, ובמחשבים תאגידיים?</strong><br>
<span dir="ltr">Office Online</span> <span dir="ltr">(Word Online, Excel Online, PowerPoint Online)</span>: אין תמיכה, הכלי דורש <span dir="ltr">WebView2</span> של <span dir="ltr">Office desktop</span>.
<span dir="ltr">Mac</span>: אין תמיכה, <span dir="ltr">Windows</span> בלבד.
מחשבים תאגידיים: יש לבדוק עם ה-<span dir="ltr">IT</span> לפני הפעלה של <span dir="ltr">debug port</span> ב-<span dir="ltr">Office</span>. הכלי לא מיועד ללפטופים תאגידיים סגורים. ראו את האזהרה למחשבים מנוהלי-ארגון בראש הקובץ.</p>
...
<ul>
  <li>פורט הדיבאג של <span dir="ltr">WebView2</span> ב-<span dir="ltr">localhost</span> (פורט דינמי) אינו מאומת - כל תהליך מקומי שרץ תחת אותו משתמש יכול להתחבר אליו. ראו "הערת אבטחה".</li>
  <li>בארגונים שמשתמשים ב-<span dir="ltr">Microsoft 365</span> עם <span dir="ltr">EDR/DLP</span>, דגל ה-<span dir="ltr">WebView2</span> עלול להיחסם. הכלי לא מיועד ללפטופים ארגוניים סגורים.</li>
  <li>עדכון של תוסף <span dir="ltr">Claude</span> שמחליף את ה-<span dir="ltr">DOM</span> עלול לשבור את ההזרקה עד שיישלח <span dir="ltr">release</span> מתוקן.</li>
  <li><strong>אין תמיכה ב-<span dir="ltr">Mac (macOS)</span> ולא צפויה תמיכה כזו.</strong> <span dir="ltr">Office</span> ל-<span dir="ltr">Mac</span> משתמש ב-<span dir="ltr">WKWebView</span> במקום <span dir="ltr">WebView2</span>, ושכבת ה-<span dir="ltr">launcher</span> כולה <span dir="ltr">(bat, vbs, ps1)</span> היא <span dir="ltr">Windows-only</span>. גם <span dir="ltr">Office Online</span> <span dir="ltr">(Word/Excel/PowerPoint Online)</span> אינו נתמך.</li>
  <li>גרסאות של תוסף <span dir="ltr">Claude</span> שלא משתמשות ב-<span dir="ltr">WebView2</span> (למשל <span dir="ltr">Electron</span> עצמאי) - אינן נתמכות.</li>
</ul>
...
<p>
דיווחי באגים ובקשות שיפור מתקבלים בברכה דרך GitHub Issues ו-Pull Requests.
</p>

<p>
אם מצאתם באג בתצוגה (לדוגמה: selector שדלף, או יישור שבור במקום מסוים),
פתחו issue חדש וצרפו:
</p>

<ul>
  <li>שחזור קצר: מה עשיתם, מה ציפיתם, ומה קרה בפועל.</li>
  <li>צילום מסך של הפאנל.</li>
  <li>קובץ <code>doctor.log</code>.</li>
</ul>
...
<p>
מפתחים עצמאיים בנו פתרונות RTL בשכבות שונות של סביבת ה-AI tooling. כל פרויקט
מטפל בסביבה שונה - אם נתקלתם בבעיה ב-Office, סביר שתפגשו אותה גם במקומות אחרים:
</p>

<ul>
  <li><strong><a href="https://github.com/GuyRonnen/rtl-for-vs-code-agents">rtl-for-vs-code-agents</a></strong> מאת גיא רונן - הרחבה ל-<span dir="ltr">VS Code</span> עבור <span dir="ltr">Claude Code, Cursor, Antigravity, and Gemini Code Assist</span> בשכבת ה-<span dir="ltr">webview</span>.</li>
  <li><strong><a href="https://github.com/noambrand/kivun-terminal-wsl">kivun-terminal-wsl</a></strong> מאת נועם ברנד - תיקון <span dir="ltr">RTL</span> בשכבת הטרמינל של <span dir="ltr">Claude Code</span> על <span dir="ltr">Windows/Linux</span>.</li>
</ul>

<p>
שני הפרויקטים האלה משלימים את הכלי הנוכחי - בחרו את הפתרון שמתאים למקום שבו
אתם נתקלים בבעיית ה-BiDi: <span dir="ltr">VS Code</span>, טרמינל,
או <span dir="ltr">Microsoft Office</span> (הכלי הזה).
</p>
...
<p>
Independent developers have built userland RTL fixes across the AI-tooling
stack. Each project tackles a disjoint surface - if you hit the BiDi problem in
Office, you're likely to hit it elsewhere too:
</p>
...
<p>
These two complement the current tool - pick the project that matches where
you're hitting the BiDi problem: VS Code, terminal, or Microsoft Office
(this tool).
</p>
