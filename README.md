...
<h3>אקטיבציה ידנית בלבד דרך Connect</h3>
<p>
החל מ-v0.1.4 אין יותר Auto-enable. בכל סשן של Word, לחצו <strong>Connect</strong>
בתפריט ה-tray כדי להפעיל RTL. ה-wrapper מגדיר את דגל ה-WebView2 רק בתהליך של
Word עצמו, ולא ככניסה קבועה ב-registry. אפליקציות WebView2 אחרות (Teams,
Outlook, Edge) לא מושפעות מההתקנה של הכלי הזה.
</p>
<p>
הסיבה לשינוי: גרסאות קודמות (v0.1.0 - v0.1.3) שמרו את הדגל כ-USER environment
variable ב-<code>HKCU\Environment</code>. המשתנה הזה נקרא ע"י כל מארח WebView2
באקאונט, מה שגרם למערכות EDR ארגוניות לסמן את ההתקנה כפעילות חשודה. v0.1.4
מסיר את המנגנון לחלוטין. שינוי שהיה במכוון - אקטיבציה דרך Connect היא הדרך
היחידה.
</p>
...
<ol>
  ...
  <li>דאבל-קליק על <strong><code>install.bat</code></strong>. ההתקנה רצה ב-4 שלבים, ללא שאלות.</li>
...
<ul>
  ...
  <li><strong>Tray-icon control</strong> - a small rounded-square icon near the clock: a white <strong>W</strong> and an RTL arrow on a status-colored background. Gray = starting. Red = not attached. Green = panel is RTL. Right-click for Connect / Disconnect / Show diagnostic log / Check for updates / Uninstall / Exit. No Start Menu entry pretending to be Word. See the <a href="docs/images/tray-icon-red.png">red (disconnected)</a> and <a href="docs/images/tray-icon-green.png">green (connected)</a> states.</li>
  <li><strong>Connect-only activation</strong> - one click in the tray per Word session. The WebView2 debug flag is set in the wrapper's process scope only, so other WebView2 apps (Teams, Outlook, Edge) are never affected.</li>
...
<h3>Why activation is Connect-only as of v0.1.4</h3>

<p>
Versions v0.1.0 - v0.1.3 offered an Auto-enable toggle that persisted
the WebView2 debug-port flag as a user-level environment variable
(<code>WEBVIEW2_ADDITIONAL_BROWSER_ARGUMENTS</code> in
<code>HKCU\Environment</code>). That variable is read by every
WebView2 host running under your user, not just Word, so changing it
affected Teams, the new Outlook, Edge WebView windows, and any other
WebView2-based app on the account. Enterprise EDR products treat
unexpected modifications of WebView2 browser arguments as a
credential-theft signal and may trigger host isolation on managed
devices. v0.1.4 removes the toggle and the persistent variable
entirely. The flag is now set per-process by <code>word-wrapper.bat</code>
when you click Connect, inherited by Word but invisible to all other
WebView2 hosts on your account.
</p>

<p>
On a v0.1.4 install over an older version, the legacy variable is
removed automatically (only when its value matches exactly what
older versions wrote; any user-modified value is preserved).
</p>
...
<ol>
  ...
  <li>Double-click <strong><code>install.bat</code></strong>. The installer runs 4 steps with no prompts.</li>
